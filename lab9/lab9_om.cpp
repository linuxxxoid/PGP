...
void WaitAll(int* coords, int* gridProc, MPI_Request *arrOfRequests)
{
	...
	for (int i = 0, j = 1; i < axes; ++i, j += 2)
	{
		if (coords[i] > 0)
		{
            // операция ожидания блокирует работу процесса до полного завершения операции
...
void Clear(double* grid, double* newGrid, double** sendBuf, double** getBuf, double* maxValues)
{
	delete[] grid;
	delete[] newGrid;
	delete[] maxValues;
...
//заполняем массив координатами соседей
	for (int i = 0, j = 1; i < axes; ++i, j += 2)
	{
		--tmp[i];
...
void Isend_Irecv(MPI_Request* in, MPI_Request* out, double** sendBuf, double** getBuf, int* sizeEdges, int* gridProc, int* coords, int* neighb)
{
	for (int i = 0, j = 1; i < axes; ++i, j += 2)
	{
        ...
        if (coords[i] > 0)
		{
            ...
            //sendbuf адрес начала расположения передаваемых данных
            // номер процесса отправителя
            
			MPI_Isend(sendBuf[i * 2], sizeEdges[i], MPI_DOUBLE,
					  neighb[i * 2], 0, MPI_COMM_WORLD, &out[i * 2]);
            //getbuf адрес принимаемых данных
            //адрес процесса получателя
...
{
	MPI_Datatype type;
	MPI_DOUBLE stride = sizeof(double);
	int tmp[axes] = {0};
	++tmp[0];
	--tmp[0];
	...
	for (int d = 0; d < directions; ++d)
	{
        ...
        // при движении по осям нужно чтобы получались след индексы в 2д
        // x = j + k * ny
        // y = i + k * nx
        // z = i + j * nx
        //при заполнении лев и прав границ
        //правая гр: for(nz)
        //              for(ny)
        // левая граница точно такая же как и правая просто друг против друга
        // для остальных осей по аналогии
...
void UpdateCoords(int& i, int& j, int& k, int shift, int* block)
{
	i += shift;
	while (i > block[0])
	{
		   ...
		   double u0, int numProcs, int rank)
{
	...
	int threadsMaxQuantity = omp_get_max_threads();
	double* maxValues = new double[threadsMaxQuantity];
	double maxConvergence = 0.0;

	double globalMax = 0.0;
	FillBuffer(maxValues, threadsMaxQuantity, 0.0);
	omp_set_dynamic(0);//запрещает динамическое изменение числа нитей в параллельной области.
	...
	do {
		...
		//Директива parallel указывает, что структурный блок кода
		//должен быть выполнен параллельно в несколько потоков.
		// Каждый из созданных потоков выполнит одинаковый код 
		//содержащийся в блоке, но не одинаковый набор команд.
		// В разных потоках могут выполняться различные ветви 
		//или обрабатываться различные данные, 
		//что зависит например от операторов if-else
		// или использования директив распределения работы.
		#pragma omp parallel
		{
			double a;
			...
			int j = 1;
			int k = 1;
			UpdateCoords(i, j, k, threadId, block);
			...
			while(k <= block[2])
			{
				...
				maxValues[threadId] = FindMax(grid[GetPos(i, j, k, block[1] + 2, block[0] + 2)], newGrid[GetPos(i, j, k, block[1] + 2, block[0] + 2)], maxValues[threadId]);
				UpdateCoords(i, j, k, threadQuantity, block);
			}

		}

		WaitAll(coords, gridProc, out);
		//получаем значения на границах для выполнения след итерации
		FillOutEdges(newGrid, sendBuf, block);

		for (int idx = 0; idx < threadsMaxQuantity; ++idx)
		{
			maxConvergence = maxValues[idx] > maxConvergence ? maxValues[idx] : maxConvergence; 
		}
		//выполнение 1 глобальной операции поиска максимума с возвратом 1 результата во всех процессах в буфере глобалмакс
	...
	Clear(grid, newGrid, sendBuf, getBuf, maxValues);
}
